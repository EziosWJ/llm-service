...
def _make_container(events=None):
    container = MagicMock()
    container.ask_service.ask_stream.return_value = iter(events or SSE_EVENTS)
    return container
...
def test_ask_stream_returns_sse_events(monkeypatch) -> None:
    ...
    response = client.post(
        "/ask/stream",
        json={"query": "What is X?", "user_id": "u1"},
    )
    ...
    assert json.loads(events[1]["data"]) == {"text": "the"}

    assert events[2]["event"] == "delta"
    assert json.loads(events[2]["data"]) == {"text": " answer"}
...
def test_ask_stream_upstream_error(monkeypatch) -> None:
